...
// 생년에 종속된 파생 기본값: 정년(만60세) 12월말 퇴직 -> 만61세부터 20년간 시뮬레이션 -> DC/개인연금·재취업·다운사이징·주택연금은 그 해(만61세)를 기본 시작연도로 사용
// 생년을 바꾸는 것은 완전히 다른 사람을 가정하는 것이므로, 아래 필드는 생년이 바뀔 때마다 사용자가 직접 수정했었는지와 무관하게 항상 이 공식으로 재계산된다.
function birthYearDerivedDefaults(birthYear) {
  const startYear = birthYear + 61; // 만 61세가 되는 해
  return {
    startYear,
    endYear: startYear + 19,
    dcStartYear: startYear,
    dcStartMonth: 1,
    personalStartYear: startYear,
    personalStartMonth: 1,
    workStartYear: startYear,
    workEndYear: startYear,
    downsizeYear: startYear,
    reverseMortgageStartYear: startYear
  };
}
const BIRTHYEAR_DERIVED_KEYS = Object.keys(birthYearDerivedDefaults(1967));

const DEFAULTS = {
  birthYear: 1967,
  dependents: 1,
  baseYear: new Date().getFullYear(), // 앱 실행 시점의 현재 연도
  inflation: 2.7,
  ...
  reverseMortgageStartMonth: 1,
  reverseMortgageMonthly: 0
};

// 필드의 "기본값"을 조회: 생년 파생 필드는 현재 state.birthYear 기준으로 매번 계산, 나머지는 DEFAULTS 고정값
function defaultOf(key, s) {
  return BIRTHYEAR_DERIVED_KEYS.includes(key) ? birthYearDerivedDefaults(s.birthYear)[key] : DEFAULTS[key];
}

let state = Object.assign({}, DEFAULTS, birthYearDerivedDefaults(DEFAULTS.birthYear));
let currentResults = [];
...
const SECTIONS = [
  {
    ...
    fields: [
      { key: 'birthYear', label: '생년(년)', type: 'number', min: 1950, max: 1990, step: 1,
        note: (s) => {
          const d = birthYearDerivedDefaults(s.birthYear);
          return `자동계산: 정년(만60세) ${s.birthYear + 60}년 12월 퇴직 → 시뮬레이션 ${d.startYear}~${d.endYear}년(만61세부터 20년) → 연금 개시 ${d.dcStartYear}년 1월. 재취업·다운사이징·주택연금 시작연도 기본값도 ${d.startYear}년으로 함께 맞춰집니다.`;
        },
        help: '만 나이 계산과 국민연금 수령 개시 연령 판정의 기준이 됩니다.\n생년을 바꾸면 완전히 다른 사람을 가정하는 것으로 간주해, 시뮬레이션 시작·종료연도, DC퇴직연금·개인연금 수령개시연도/월, 재취업 시작·종료연도, 다운사이징 연도, 주택연금 가입연도(총 10개 필드)가 "정년 만60세 12월 퇴직 → 만61세부터 20년간 시뮬레이션, 연금·재취업·다운사이징·주택연금은 만61세부터 시작" 공식으로 항상 자동 재계산됩니다. 해당 필드들을 직접 수정해두었더라도 생년을 바꾸면 그 값은 덮어써집니다.' },
      { key: 'dependents', label: '인적공제 대상자 수(본인 포함)', type: 'range', min: 1, max: 6, step: 1, unit: '명',
        help: '종합소득세 계산 시 적용되는 인적공제 대상자 수입니다(본인 포함).\n근거: 소득세법 제50조(인적공제)\n산출: 종합소득금액 - (150만원 × 인원수) = 과세표준\n참고: 만 70세 이상에게 적용되는 경로우대공제(소득세법 제51조, 1인당 연 100만원)는 월별 현금흐름에 미치는 영향이 작아 이 시뮬레이터에는 반영하지 않았습니다. 실제로는 이 금액만큼 세금이 더 낮아질 수 있습니다.' },
      { key: 'startYear', label: '시뮬레이션 시작연도', type: 'number', min: 2020, max: 2050, step: 1,
        help: '현금흐름을 계산할 기간의 시작연도입니다.\n생년으로부터 "정년(만60세) 12월 퇴직 → 만61세부터 20년간" 공식으로 자동계산된 값이 기본으로 채워지며, 직접 수정할 수 있습니다. 단, 생년을 다시 바꾸면 이 값은 새 공식값으로 덮어써집니다.' },
      { key: 'endYear', label: '시뮬레이션 종료연도', type: 'number', min: 2020, max: 2060, step: 1,
        help: '현금흐름을 계산할 기간의 종료연도입니다. 시작연도보다 빠르면 계산되지 않습니다.\n생년 기준 자동계산값(시작연도+19년)이 기본으로 채워지며, 직접 수정할 수 있습니다. 단, 생년을 다시 바꾸면 이 값은 새 공식값으로 덮어써집니다.' },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'workStartYear', label: '재취업 시작 연도', type: 'number', min: 2020, max: 2060,
        help: '재취업 근무 시작 연도입니다. 임의계속가입 자격(12개월 이상 재직) 판정에도 사용됩니다.\n생년 기준 자동계산값(만61세가 되는 해)이 기본으로 채워지며, 직접 수정할 수 있습니다. 단, 생년을 다시 바꾸면 이 값은 새 공식값으로 덮어써집니다.' },
  ...
  },
  {
    idx: '07', title: '주택연금',
    fields: [
      { key: 'downsizeEnabled', label: '다운사이징 반영 (재산세·종부세·건보료에만 반영, 양도차익/양도세는 현금흐름에 미포함)', type: 'checkbox',
        help: '매도 시 발생하는 양도차익·양도소득세는 1회성 거액이라 월별 현금흐름 왜곡을 피하기 위해 의도적으로 제외했습니다. 재산세·종합부동산세·건강보험 재산분에만 반영됩니다.\n아래 주택연금과 함께 켤 수 있습니다. 예: 먼저 다운사이징한 뒤 낮아진 집으로 주택연금에 가입하려면, 다운사이징 연도를 주택연금 가입 연도보다 앞선 시점으로 설정하세요. 그러면 주택연금 가입 연도부터는 다운사이징 후 공시가격 기준으로 재산세 감면이 계산됩니다.' },
      { key: 'downsizeYear', label: '다운사이징 연도', type: 'number', min: 2020, max: 2060,
        help: '이 연도부터 낮아진 공시가격 기준으로 재산세·종부세·건강보험 재산분이 재계산됩니다.\n생년 기준 자동계산값(만61세가 되는 해)이 기본으로 채워지며, 직접 수정할 수 있습니다. 단, 생년을 다시 바꾸면 이 값은 새 공식값으로 덮어써집니다.' },
      { key: 'downsizeTargetGongsiga', label: '다운사이징 후 공시가격(현재가로 입력)', type: 'number', unit: '만원',
        help: '다운사이징 후 주택의 공시가격입니다. 위 "주택 공시가격"과 마찬가지로 오늘(공시가격 기준연도) 기준 가격을 입력하세요 — 다운사이징 연도가 아닌 기준연도부터 물가상승률을 반영해 자동으로 미래 시점 금액으로 환산됩니다.' },
      { key: 'reverseMortgageEnabled', label: '주택연금(역모기지) 가입 반영', type: 'checkbox',
        help: '집을 팔지 않고 담보로 제공한 뒤 매월 연금(대출)을 받는 방식입니다. 아래에서 입력한 월 수령액이 현금흐름에 더해지고, 가입 연도부터 재산세가 감면됩니다.\n위 다운사이징과 함께 켤 수 있습니다. 다운사이징을 먼저(더 이른 연도로) 설정해두면, 주택연금의 재산세 감면 계산도 다운사이징 후 낮아진 공시가격을 기준으로 자동 반영됩니다.\n근거: 한국주택금융공사법(주택연금), 지방세특례제한법 제289조(재산세 감면)' },
      { key: 'reverseMortgageStartYear', label: '주택연금 가입 연도', type: 'number', min: 2020, max: 2060,
        help: '이 연도·월부터 월 수령액이 지급되고 재산세 감면이 시작됩니다.\n생년 기준 자동계산값(만61세가 되는 해)이 기본으로 채워지며, 직접 수정할 수 있습니다. 단, 생년을 다시 바꾸면 이 값은 새 공식값으로 덮어써집니다.' },
      { key: 'reverseMortgageStartMonth', label: '주택연금 가입 월', type: 'number', min: 1, max: 12,
        help: '주택연금 가입이 시작되는 월입니다.' },
      { key: 'reverseMortgageMonthly', label: '주택연금 월 수령액', type: 'number', unit: '만원',
        help: '한국주택금융공사 홈페이지의 "예상연금 조회"에서 확인한 월 지급금을 입력합니다(가입연령·주택가격·지급방식에 따라 달라져 자동계산하지 않습니다). 다운사이징 후에 가입하는 경우, 다운사이징된 주택가격을 기준으로 조회한 금액을 입력해야 합니다. 대출 성격이라 소득세·건강보험료 부과대상 소득에는 포함하지 않고 현금흐름에만 더합니다. 물가상승률에 따른 자동 인상은 반영하지 않습니다(지급방식에 따라 정액 또는 체증 등 다양하여 사용자가 조정 필요).\n[재산세 감면] 시가표준액 5억원 이하 주택은 재산세 25% 감면, 5억원 초과 주택은 5억원 상당 재산세액의 25%까지만 감면됩니다(1가구1주택자, 2027.12.31까지 한시 적용). 종합부동산세는 별도 감면이 없습니다.' }
    ]
  }
];
...
function renderSidebar() {
  ...
  SECTIONS.forEach(sec => sec.fields.forEach(f => {
    ...
    el.addEventListener('input', () => {
      ...
      if (f.key === 'birthYear') applyBirthYear(v);
      else state[f.key] = v;
      if (f.type === 'range') {
...
// 필드 정의를 key로 조회 (SECTIONS 전체에서 검색)
...
// state[key] 변경을 해당 필드의 DOM 입력에도 반영 (사용자 조작 없이 값이 바뀔 때 사용)
function syncFieldInput(key) {
  const f = findFieldDef(key);
  const el = document.getElementById('f_' + key);
  if (!f || !el) return;
  el.value = state[key];
  if (f.type === 'range') {
    const v = document.getElementById('v_' + key);
    if (v) v.textContent = state[key] + (f.unit || '');
  }
}

// 생년 변경 = 완전히 다른 사람을 가정하는 것이므로, 생년 파생 6개 필드를 사용자가 직접 수정했는지와 무관하게 항상 새 공식값으로 재설정
function applyBirthYear(newBirthYear) {
  state.birthYear = newBirthYear;
  const derived = birthYearDerivedDefaults(newBirthYear);
  BIRTHYEAR_DERIVED_KEYS.forEach(k => {
    state[k] = derived[k];
    syncFieldInput(k);
  });
...
function resetField(f) {
  if (f.key === 'birthYear') applyBirthYear(defaultOf('birthYear', state));
  else state[f.key] = defaultOf(f.key, state);
  const el = document.getElementById('f_' + f.key);
...
function updateResetUI() {
  SECTIONS.forEach((sec, si) => {
    ...
    sec.fields.forEach(f => {
      const changed = state[f.key] !== defaultOf(f.key, state);
...
function renderTimeline(results) {
  ...
  results.forEach(r => {
    ...
    if (r.reverseMortgage > 0) { phase = 'phase-reversemortgage'; label = '주택연금'; }
    if (r.isSaleYear) { phase = 'phase-downsize'; label = r.reverseMortgage > 0 ? '다운사이징·주택연금' : '다운사이징'; }
    if (r.npsAnnual > 0) { phase = 'phase-nps'; label = r.reverseMortgage > 0 ? '국민연금·주택연금' : '국민연금'; }
...
// 연도별 차트 (Canvas)
// ------------------------------------------------------------
// 월별 수입/지출 막대의 Y축 상한: 실제 최대값을 덮는 첫 고정 단계를 선택해 매번 눈금이 요동치지 않도록 함
const CHART_AXIS_TIERS = [1000, 2000, 3000, 5000, 10000];
function pickAxisTier(maxVal) {
  const tier = CHART_AXIS_TIERS.find(t => t >= maxVal);
  if (tier !== undefined) return tier;
  return Math.ceil(maxVal / 10000) * 10000; // 1억원을 넘는 초고액 시나리오는 다음 1억원 단위로 올림
}
...
function renderChart(results) {
  ...
  // Y축 상한은 실제 최대값을 그대로 쓰지 않고 고정 단계(1000/2000/3000/5000/10000만원…)로 스냅해
  // 입력값을 조금만 바꿔도 눈금이 흔들리지 않고 시나리오 간 비교가 쉽도록 함
  const barValues = [...incomes, ...expenses];
  const maxTop = pickAxisTier(Math.max(...barValues, 0));

  // 순현금흐름이 적자인 달만을 위한 하단 여백(적자는 항상 지출 이하이므로 maxTop을 넘지 않음)
  const deficits = nets.map(v => Math.max(0, -v));
  const maxBottom = Math.min(Math.max(...deficits, 0), maxTop);
...
function renderMonthlyTable(results) {
  ...
  const rowsDef = [
    ...
    ['국민연금', 'nps'],
    ['근로소득', 'earned'], ['실업급여', 'unemployment'], ['주택연금', 'reverseMortgage']
  ];
...
주택 다운사이징과 주택연금(역모기지)은 <b>함께 반영할 수 있습니다</b>. 다운사이징은 <b>재산세·종합부동산세·건강보험 재산분에만 반영</b>됩니다. 매도 시 발생하는 양도차익·양도소득세는 1회성 거액이라 월별 현금흐름 왜곡을 피하기 위해 의도적으로 제외했습니다(다운사이징 연도부터 낮아진 공시가격 기준으로 세금·건보료만 계속 반영).
    주택연금은 집을 팔지 않고 담보로 제공해 매월 대출 형태로 연금을 받는 방식으로, 입력한 <b>월 수령액이 그대로 현금흐름에 더해지고 소득세·건강보험료 부과대상 소득에는 포함되지 않습니다</b>. 정확한 월 지급액은 가입연령·주택가격·지급방식에 따라 달라지므로 한국주택금융공사 홈페이지의 예상연금 조회 결과를 직접 입력해야 하며, 물가상승에 따른 자동 인상은 반영하지 않습니다. 가입 시점부터 <b>재산세가 감면</b>됩니다: 시가표준액 5억원 이하는 재산세 전액의 25%, 5억원 초과는 5억원 상당 재산세액의 25%까지만 감면됩니다(1가구1주택자 대상, 2027.12.31까지 한시 적용, 종합부동산세는 별도 감면 없음). 두 항목을 함께 켜고 다운사이징 연도를 주택연금 가입 연도보다 앞서 설정하면, 주택연금 가입 이후의 재산세 감면도 다운사이징 후 낮아진 공시가격을 기준으로 자동 계산됩니다(월 수령액 자체는 다운사이징 후 주택가격 기준으로 사용자가 직접 조회해 입력해야 합니다).
...
function buildReport() {
  ...
  let html = `
    ...
    <table class="rp-settings">
      ...
      <tr><td>주택연금</td><td colspan="3">${state.reverseMortgageEnabled ? `${state.reverseMortgageStartYear}년 ${state.reverseMortgageStartMonth}월 가입, 월 ${fmt(state.reverseMortgageMonthly)}만원 수령` : '미반영'}</td></tr>
    </table>
    ...
    <table>
      <tr><th>연도</th><th>나이</th><th>DC연금</th><th>개인연금</th><th>국민연금</th><th>근로소득</th><th>실업급여</th><th>주택연금</th><th>퇴직소득세</th><th>종합/분리세</th><th>기타소득세</th><th>건보+장기요양</th><th>건보유형</th><th>재산세</th><th>종부세</th><th>총세금</th><th>총수입</th><th>세후순수입</th></tr>
      ${currentResults.map(r => `<tr>
        <td>${r.year}</td><td>${r.age}</td><td>${fmt(r.dcAnnual)}</td><td>${fmt(r.personalAnnual)}</td><td>${fmt(r.npsAnnual)}</td>
        <td>${fmt(r.earned)}</td><td>${fmt(r.unemployment)}</td><td>${fmt(r.reverseMortgage)}</td><td>${fmt(r.taxRetire)}</td><td>${fmt(r.taxIncome)}</td><td>${fmt(r.taxOtherIncome)}</td>
      ...
      <tr style="font-weight:700;background:#f0f0ea;">
        ...
        <td>${fmt(currentResults.reduce((s, r) => s + r.reverseMortgage, 0))}</td>
        <td>${fmt(currentResults.reduce((s, r) => s + r.taxRetire, 0))}</td>
        <td>${fmt(currentResults.reduce((s, r) => s + r.taxIncome, 0))}</td>
        <td>${fmt(currentResults.reduce((s, r) => s + r.taxOtherIncome, 0))}</td>
    ...
    <div class="rp-assumptions">
      ...
      <div>주택 다운사이징과 주택연금(역모기지)은 함께 반영할 수 있습니다. 다운사이징은 재산세·종합부동산세·건강보험 재산분에만 반영되며, 양도차익·양도소득세는 1회성 거액이라 현금흐름에서 제외했습니다.</div>
      <div>주택연금은 집을 팔지 않고 담보로 제공해 매월 대출 형태로 연금을 받는 방식으로, 입력한 월 수령액이 현금흐름에 더해지고 소득세·건강보험료 부과대상 소득에는 포함되지 않습니다. 정확한 월 지급액은 한국주택금융공사 홈페이지의 예상연금 조회 결과를 직접 입력해야 하며 물가상승 자동 인상은 반영하지 않습니다. 가입 시점부터 재산세가 감면됩니다: 시가표준액 5억원 이하는 재산세 전액의 25%, 5억원 초과는 5억원 상당 재산세액의 25%까지만 감면됩니다(1가구1주택자, 2027.12.31까지 한시 적용, 종부세는 별도 감면 없음). 다운사이징을 함께 켜고 다운사이징 연도를 주택연금 가입 연도보다 앞서 설정하면, 주택연금 재산세 감면도 다운사이징 후 낮아진 공시가격을 기준으로 계산됩니다.</div>
